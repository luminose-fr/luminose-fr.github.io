...
var App = {
  ...
  init(config) {
    ...
    this._config = {
      ...
      urls: {
        ...
        stripe: {
          make_webhook_get_payment_intent: 'https://hook.eu1.make.com/fwj3qqn7fcpnyo7m2anjw21esql2292k',
          make_webhook_after: '/api/validate-form',
        }
      }
    };
  ...
  setupFormValidation: function() {
    if (document.querySelector('#form-questionnaire-sante') !== null) {
      ...
      var that                = this;
      var urlEtapeDeux        = window.location.origin + "/respiration-holotropique/inscription-etape-2.html";
  ...
  _loadStripeElements: async function(optionsPaiements) {
    ...
    const queryString = window.location.search; // Récupère tout ce qui est après "?"
    const urlParams = new URLSearchParams(queryString); // Crée un objet pour manipuler les paramètres
    const notionPageId = urlParams.get("notion_page_id");
    const billingDetails = {};
    const billingDetailsFields = {};
    const email = decodeURIComponent(urlParams.get("email"));
    const nom = decodeURIComponent(urlParams.get("nom"));
    const prenom = decodeURIComponent(urlParams.get("prenom"));
    const telephone = decodeURIComponent(urlParams.get("telephone"));
    const ville = decodeURIComponent(urlParams.get("ville"));
    const codePostal = decodeURIComponent(urlParams.get("code_postal"));
    if (email != null && email != '' && email != "null") {
      billingDetails.email = email;
      billingDetailsFields.email= 'never';
    }
    if (nom != null && nom != '' && nom != "null" && prenom != null && prenom != '' && prenom != "null") {
      billingDetails.name = prenom + " " + nom;
      billingDetailsFields.name= 'never';
    }
    if (telephone != null && telephone != '' && telephone != "null") {
      billingDetails.phone = telephone;
    ...
    if (ville != null && ville != '' && ville != "null") {
      billingDetails.address.city = ville;
    }
    if (codePostal != null && codePostal != '' && codePostal != "null") {
      billingDetails.address.postal_code = codePostal;
    }   

    // Metadata (données personnalisées pour Stripe)
    const metadata = {
      notion_page_id: notionPageId
    };

    // Créer le PaymentIntent via Make.com
    const { error: backendError, clientSecret } = await fetch(make_webhook_get_payment_intent, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
      body: JSON.stringify({
        amount: 16000, // Montant en centimes (par ex. 10,00€)
        currency: "eur",
        metadata: metadata
      }),
    }).then(r => r.json());
    ...
    form.addEventListener('submit', async (e) => {
      ...
      // Confirm the payment
      console.log("sending billingDetails = ");
      console.log(billingDetails);
      const { error: stripeError } = await stripe.confirmPayment({
        elements,
        confirmParams: {
          return_url: `${window.location.origin}/respiration-holotropique/inscription-etape-3.html`,
          payment_method_data: {
